...
\begin{document}
\begin{abstract}
We prove Nivat's conjecture: given a finite alphabet $A$ and a map $c:\Z^2\to A$, if $c$ has at most $mn$ distinct $m\times n$ rectangular patterns for some positive integers $m,n$, then $c$ is invariant under a nonzero translation. After embedding $A$ in $\Q$, we induct on rectangle area. The key step applies a Laurent polynomial in the lattice shifts to $c$ and finds a smaller $r\times s$ rectangle on which the resulting map has at most $rs$ distinct patterns. The polynomial is supported on a line and generates the annihilator ideal of a nonzero difference of two pointwise limits of translates of $c$. This difference vanishes on a half-plane and is periodic parallel to its boundary. The induction reduces the conjecture to the case where a product of two difference operators annihilates $c$, which was proved by Szabados. The full argument is formalized in Lean using mathlib.
\end{abstract}
\maketitle
