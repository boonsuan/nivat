...
\hypersetup{pdftitle={Pattern complexity and Nivat's conjecture},pdfsubject={Rectangular pattern complexity and polynomial annihilators},pdfauthor={}}
...
% Author metadata is omitted; no authorship or affiliation is asserted.
...
\begin{document}
\begin{abstract}
We prove Nivat's conjecture: a finite-alphabet configuration on $\Z^2$ with at most $mn$ distinct $m\times n$ patterns has a nonzero period. The proof reduces the area of a rectangle on which this complexity bound holds. Its main estimate concerns a linear combination of translates of the configuration. The corresponding map on rectangular patterns loses some coordinates, but, under an exact annihilator hypothesis, it identifies at least as many input patterns as the number of lost coordinates. We obtain this hypothesis from the difference of two orbit-closure points that agree on a half-plane. A minimal polynomial in the direction parallel to the boundary generates the entire annihilator ideal of this difference. The reduction ends with a configuration annihilated by two difference operators. We include a proof of periodicity in this case, following the boundary-extension method of Cyr--Kra and Szabados and using a finite recurrence to extend periodicity from a strip.
\end{abstract}
\maketitle
